...
function pintarRespuesta(respuesta) {
  ...
  <table class='table table-sm table-responsive' style="height: auto; width: auto">
      <thead>
          <th style='width: auto; background-color: #99D9F2; color: white; ' scope='col'
              align='center'>
              <center>ID</center>
          </th>
          ...
          <th style='width: auto; background-color: #99D9F2; color: white; line-height: 100%'
 ...
 `;
    ...
    if (respuesta.length < 1) {
        ...
        for (i = 0; i < respuesta.length; i++) {
            ...
            sessionStorage.setItem('reference', respuesta[i].id)

            

            myTable += "<td align='center'><button data-toggle='modal' data-target='#MymodalUpdate' class='btn btn-outline-success' onclick='consultarProduct(" + respuesta[i].price + ",\"" + respuesta[i].id + "\")'> <img src='https://image.flaticon.com/icons/png/512/104/104668.png' width='50' height='30'>EDIT</button></td>";

            myTable += "<td align='center'><button class='btn btn-outline-danger' onclick='deleteProduct(" + respuesta[i].price + ",\"" + respuesta[i].id + "\")'> <img src='https://img.icons8.com/wired/64/000000/filled-trash.png' width='30' height='30'/>DELETE</button></td>";
...
async function autoInicioProducts() {
    ...
    try {
        const url = 'http://129.146.29.203:8080/api/cleaningprod/all'
        // console.log(url)
...
async function consultarProduct(price, id) {
...
async function createProduct() {
    ...
    if ($("#reference").val().length == 0 ||
        ...
        try {
            debugger
            let availability;
            const urlUpdate = 'http://129.146.29.203:8080/api/cleaningprod/new'
...
async function updateProduct() {
    ...
    ) 
    {
        ...
        try {
            ...
            const urlUpdate = 'http://129.146.29.203:8080/api/cleaningprod/update'
...
async function deleteProduct(price, reference) {
    ...
    try {
        ...
        const urlUpdate = 'http://129.146.29.203:8080/api/cleaningprod/' + reference
